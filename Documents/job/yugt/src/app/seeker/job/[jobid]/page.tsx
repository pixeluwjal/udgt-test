    ...
    <div className="flex h-screen bg-gradient-to-br from-blue-50 to-indigo-100 overflow-hidden font-inter">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="md:hidden bg-white shadow-sm p-4 flex justify-between items-center">
          <span className="text-lg font-bold text-indigo-600">Job Details</span>
        </div>

        <main className="flex-1 overflow-y-auto p-4 md:p-8">
          <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-8">
            {/* Main Job Details Section */}
            <div className="flex-1 bg-white rounded-xl shadow-md border border-gray-100 p-6 lg:p-8">
              <div className="flex items-start justify-between mb-6">
                <div className="flex items-center gap-4">
                  <div className="flex-shrink-0 h-16 w-16 bg-indigo-100 rounded-lg flex items-center justify-center shadow-sm">
                    {job?.companyLogo ? (
                  ...
                  <div>
                    <h1 className="text-2xl md:text-3xl font-extrabold text-gray-900">{job?.title || 'Loading Job...'}</h1>
                    <p className="text-lg text-gray-600 mt-1">{job?.company || 'N/A'} • {job?.location || 'N/A'}</p>
                  </div>
                </div>
                <div className="text-right">
                  <span className="text-2xl font-bold text-gray-900">${job?.salary.toLocaleString() || 'N/A'}</span>
                ...
                <>
                    ...
                    <div className="mb-6">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {job.skills.map((skill, index) => (
                          <span key={index} className="px-4 py-1 bg-indigo-100 text-indigo-800 text-sm rounded-full font-medium">
                            {skill}
                          </span>
                        ))}
                      </div>
                  ...
                  <div className="mb-6">
                    ...
                    <ul className="space-y-2 text-gray-700">
                      ...
                      <li className="flex items-center">
                        <svg className="w-5 h-5 mr-2 text-indigo-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8V6m0 6v2m0 6v2" /></svg>
                        <strong>Salary:</strong> ${job.salary.toLocaleString()} / year
                      </li>
                      <li className="flex items-center">
                        <svg className="w-5 h-5 mr-2 text-indigo-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
                  ...
                  <div className="mt-8 flex flex-col sm:flex-row gap-4">
                    ...
                    {applicationStatus === 'Applying...' ? (
                      <button
                        disabled
                        className="flex-1 px-6 py-3 bg-indigo-300 text-white rounded-md font-medium cursor-not-allowed shadow-md flex items-center justify-center gap-2"
                      >
                      ...
                      <button
                        disabled
                        className="flex-1 px-6 py-3 bg-green-100 text-green-800 rounded-md font-medium flex items-center justify-center gap-2 shadow-md"
                      >
                      ...
                      <button
                        onClick={handleApply}
                        className="flex-1 px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-md font-medium transition-colors duration-200 shadow-md flex items-center justify-center gap-2"
                      >
                    ...
                    <button
                      ...
                      className={`flex-1 px-6 py-3 rounded-md font-medium transition-colors duration-200 shadow-md flex items-center justify-center gap-2
                        ${job.isSaved
            ...
            <div className="w-full lg:w-80 flex-shrink-0 bg-white rounded-xl shadow-md border border-gray-100 p-6">
              <h2 className="text-xl font-extrabold text-gray-800 mb-4">More Jobs Like This</h2>
              {loadingMoreJobs ? (
                ...
                <div className="space-y-4">
                  {moreJobs.map((otherJob) => (
                    <Link href={`/seeker/job/${otherJob._id}`} key={otherJob._id} passHref>
                      <div className="block p-4 rounded-lg border border-gray-200 hover:bg-gray-50 transition-colors duration-200 cursor-pointer">
